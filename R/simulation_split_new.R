simulation_split_new <- function(W, Z, y, prior = "Dirichlet", SS = TRUE, N = 1e5, gamma_hyperprior = TRUE,
    ...
    if(prior == "Dirichlet"){
        ...
        psi <- matrix(rep(-0.1, d1*M), ncol = d1)
        psi_post <- list(0)
        b_psi <- 1
        b_psi_post <- rep(1, N)
        ...
        for(i in 1:N){
            ...
            if(k == 1){
                if(k < M){
                    ...
                    psi <- rbind(psi, matrix(0, nrow = M-k, ncol = d1))
                    ...
                    for(m in 1:k){
                        ...
                        #update \alpha
                        alpha[m] <- rnorm(1, (a_alpha*b_alpha + lambda*sum(y[V_m] - W[V_m, , drop=F] %*% psi[m,] - Z[V_m, , drop=F] %*% Beta[m, ]))/
                        ...
                        for(d in 1:d2){
                            ...
                            C_md <- sqrt(tau/(tau + lambda*sum(Z[V_m, d]^2)))*exp(-0.5*tau*mu^2)*
                                exp(((mu*tau + lambda*sum((y[V_m] - alpha[m] - W[V_m,  , drop=F] %*% psi[m,] - Z[V_m, -d , drop=F] %*% Beta[m, -d])*
                                                              Z[V_m, d]))^2)/(2*(tau + lambda*sum(Z[V_m, d]^2))))
                            theta_md <- rbinom(1, 1, w[d]/(w[d] + (1 - w[d])*C_md))
                            Beta[m, d] <- ifelse(theta_md == 1, 0, rnorm(1, (lambda*sum((y[V_m] - alpha[m] - W[V_m,  , drop=F] %*% psi[m,]
                                                                                         - Z[V_m, -d , drop=F] %*%
                                                                                             Beta[m, -d])*Z[V_m, d]) +
                        ...
                        #update \psi
                        var_mtx <- solve(lambda*(t(W[V_m, ,drop=F]) %*% W[V_m, ,drop=F]) + diag(b_psi, d1))
                        psi[m, ] <- mvrnorm(1, var_mtx %*% (colSums(sweep(W[V_m, ,drop=F], MARGIN = 1, (y[V_m] - alpha[m] - Z[V_m, ,drop=F] %*% Beta[m, ]), "*")) *
                    ...
                    for(m in (k+1):M){
                        ...
                        psi[m, ] <- rnorm(d1, a_psi, 1/sqrt(b_psi))
                    }
            ...
            lambda <- rgamma(1, a_lambda + n/2, b_lambda + sum((y-alpha[c]-apply(psi[c,]*W, 1, sum)-apply(Beta[c,]*Z, 1, sum))^2)/2)
            ...
            #update \b_psi
            b_psi <- rgamma(1, a_4+M*d1/2, b_4+sum((psi - a_psi)^2)/2)
